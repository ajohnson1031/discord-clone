    ...
    <div className="relative group flex items-center hover:bg-black/5 p-4 transition w-full">
      <div className="group flex gap-x-2 items-start w-full">
        ...
        <div className="flex flex-col w-full">
          ...
          {!fileUrl && !isEditing && (
            <p className={cn("text-sm text-zinc-600 dark:text-zinc-300", { "italic text-zinc-500 dark:text-zinc-400 text-xs mt-1": deleted })}>
              {content}
              {isUpdated && !deleted && <span className="mx-2 text-zinc-500 dark:text-zinc-400">(edited)</span>}
            </p>
          )}
          {!fileUrl && isEditing && (
            <Form {...form}>
